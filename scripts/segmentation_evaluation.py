...
import pandas as pd
import torch
from monai.metrics import DiceMetric, MeanIoU, get_confusion_matrix


class ConfusionMatrix(object):
    def __call__(self, pred, gt):
        return get_confusion_matrix(pred, gt, include_background=False)
...
def main():
    ...
    metrics = [DiceMetric(), MeanIoU(), ConfusionMatrix()]
    ...
    for gt in gts:
        ...
        for label, pred_mask, gt_mask in classwise_pairs:
            ...
            for metric in metrics:
                value = compute_metrics(pred_mask.unsqueeze(0), gt_mask.unsqueeze(0), metric)
